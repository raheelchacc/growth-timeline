                    ...
                    <ul className="space-y-3">
                        {history.map((item) => (
                            <li key={item.id}>
                                <button
                                    onClick={() => onLoad(item)}
                                    className="w-full text-left p-3 bg-slate-700/50 hover:bg-slate-700 rounded-md transition-colors duration-150"
                                >
...
const App = () => {
    ...
    const generateTimeline = async () => {
        ...
        if (!isAuthReady || !auth || !db) {
            setError("Application is not ready. Please wait and try again.");
            return;
        }
    ...
    const loadTimelineFromHistory = (historyItem) => {
        ...
        setIsHistoryVisible(false); // Close panel after loading
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-slate-100 p-4 sm:p-6 md:p-8 font-sans">
            ...
            <HistoryPanel 
                ...
                toggleVisibility={() => setIsHistoryVisible(!isHistoryVisible)}
            />
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g., SaaS for small businesses"
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows="3"
                                placeholder="e.g., Startup, 1 yr old, $100k ARR"
                                className="w-full px-4 py-2.5 bg-slate-700/80 border border-slate-600 rounded-lg text-slate-100 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 transition-all duration-150 shadow-sm hover:border-slate-500"
                            />
                        </div>
                        <div>
                        ...
                        {!timelineData && !isLoading && (
                            <div className="text-center md:text-left">
                                ...
                                <p className="text-slate-300/80 text-sm leading-relaxed">
                                    Fill in your business details, then click the button to generate a personalized growth timeline.
                                </p>
                            </div>
                        )}
                        {isLoading && (
