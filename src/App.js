...
import { getFirestore, collection, addDoc, query, onSnapshot, doc, serverTimestamp, setDoc } from 'firebase/firestore';
import { Briefcase, Target, CalendarDays, Zap, CheckCircle, AlertTriangle, Loader2, Save, ListChecks, Lightbulb, Users, BarChart2, Info, Brain, ChevronsRight } from 'lucide-react';
...
const App = () => {
   ...
   const unsubscribe = onAuthStateChanged(firebaseAuth, async (user) => {
     if (user) {
       ...
       try {
         ...
         setError("Authentication failed. Please ensure Firebase is configured correctly.");
       }
     }
     setIsAuthReady(true);
   });
 ...
 const generateTimeline = async () => {
   ...
   try {
     ...
     if (result.candidates && result.candidates.length > 0 &&
       ...
       await saveTimelineToFirestore(parsedJson); // Save after successful generation
     } else {
 ...
 const saveTimelineToFirestore = async (dataToSave) => {
   if (!db || !userId || !isAuthReady) {
     setError("Firestore is not ready or user is not authenticated. Cannot save timeline.");
   ...
   try {
     ...
     // Using a consistent ID based on inputs + timestamp could prevent exact duplicates if desired,
     // but for simplicity, we'll use a new generated ID each time.
     ...
     await setDoc(timelineDocRef, {
       ...
       businessType,
       businessPosition,
       timeline: dataToSave, // Save the structured JSON from the LLM
       createdAt: serverTimestamp(),
       appId: appId, // Store appId for context
       userId: userId, // Store userId for context
     });
     // console.log("Timeline saved to Firestore successfully!");
     // Optionally, add a success message to the UI:
     // setError("Timeline generated and saved successfully!"); // Using error state for success message for simplicity
   } catch (e) {
     console.error("Error saving timeline to Firestore:", e);
     setError("Failed to save timeline to database. Please try again.");
   }
 };
 ...
 const MessageModal = ({ message, type, onClose }) => {
   ...
   return (
     <div className={`fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-[100]`}> {/* Increased z-index */}
       <div className={`rounded-xl shadow-2xl p-6 w-full max-w-md ${type === 'error' ? 'bg-red-800/90 border-red-600' : 'bg-sky-700/90 border-sky-500'} text-slate-100 backdrop-blur-md`}>
         <div className="flex items-start">
           ...
           <div className="flex-1">
             <h3 className={`text-xl font-semibold ${type === 'error' ? 'text-red-200' : 'text-sky-200'}`}>
         ...
         <div className="mt-6 text-right">
           <button
             onClick={onClose}
             className="px-5 py-2 bg-slate-600 text-slate-100 text-sm font-medium rounded-lg hover:bg-slate-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-current_bg focus:ring-slate-400 transition-colors"
           >
             Close
           </button>
         </div>
